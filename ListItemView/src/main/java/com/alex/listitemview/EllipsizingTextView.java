...
public class EllipsizingTextView extends TextView {
    ...
    private abstract class EllipsizeStrategy {
        ...
        public boolean isInLayout(CharSequence text) {
            return isInfinity() || createWorkingLayout(text).getLineCount() <= getLinesCount();
        }
    ...
    private class EllipsizeMiddleStrategy extends EllipsizeStrategy {
        ...
        protected CharSequence createEllipsizedText(CharSequence fullText) {
            ...
            int cutOffLength = length - cutOffIndex;
            if (cutOffLength < ELLIPSIS.length()) cutOffLength = ELLIPSIS.length();
            cutOffLength += cutOffIndex % 2;    // Make it even.
            String firstPart = TextUtils.substring(fullText, 0, length / 2 - cutOffLength / 2).trim();
            String secondPart = TextUtils.substring(fullText, length / 2 + cutOffLength / 2, length).trim();
            ...
            if (fullText instanceof Spanned) {
                ...
                TextUtils.copySpansFrom((Spanned) fullText, length - secondPart.length(), length, null, secondDest, 0);
            }
