...
public class StrongOkHttpClientBuilder extends StrongBuilderBase<StrongOkHttpClientBuilder, OkHttpClient> {
     ...
     * @throws Exception
     */
    static public StrongOkHttpClientBuilder forMaxSecurity(Context context) throws Exception {
        return(new StrongOkHttpClientBuilder(context).withBestProxy());
    }
    ...
    public OkHttpClient.Builder applyTo(OkHttpClient.Builder builder, Intent status) {
        ...
        if(factory!=null){builder.sslSocketFactory(factory);}
    ...
    protected String get(Intent status, OkHttpClient connection, String url) throws Exception {
        return(connection.newCall(new Request.Builder().url(TOR_CHECK_URL).build()).execute().body().string());
    }
}